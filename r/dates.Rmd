...
as_datetime("2011-06-04 11:30:00", tz = "America/Los_Angeles")
...
mdy_hms("06042011 12:00:00", tz = "America/Los_Angeles")
...
parse_date_time(c("2011-06-04 11:30:00", "June/4/11 11:30 PM"), orders = c("%Y-%m-%d $H:%M:%S", "%B/%d/%y %I:%M %p"), tz = "America/Los_Angeles")
parse_date_time("2011-06-04 11:30:00", "%Y-%m-%d $H:%M:%S") %>% str()
parse_date_time("2011-06-04 11:30:00", "%Y-%m-%d $H:%M:%S") %>% tz()
parse_date_time("2011-06-04 11:30:00", "%Y-%m-%d $H:%M:%S", tz = "America/Los_Angeles") %>% tz()

# Not Lubridate, but still from tidyverse (readr)
parse_datetime("2011-06-04 11:30:00") 
parse_datetime("2011-06-04 11:30:00", locale = locale(tz = "America/Los_Angeles")) 
...
as.POSIXct("2011-06-04 11:30:00", tz = "America/Los_Angeles")
...
as.POSIXlt("2011-06-04 11:30:00", tz = "America/Los_Angeles")
...
strptime("2011-06-04 11:30:00", format = "%Y-%m-%d %I:%M:%S", tz = "America/Los_Angeles")
...
make_datetime(2011, 6, 2, 00, 00, 00, tz = "America/Los_Angeles")
...
simpleDatetimesCSV <- read_csv(file.path(dataDir, "datetimes-simple.csv"), 
  ...
  mutate(
    datetime = parse_datetime(datetime, locale = locale(tz = "America/Los_Angeles"))
  )
...
altDatetimesCSV <- read_csv(file.path(dataDir, "datetimes-alt.csv"), 
  ...
  mutate(
    datetime = parse_datetime(datetime, format = "%b.%d.%Y %H:%M", locale = locale(tz = "America/Los_Angeles"))
  )
...
numericDatetimesCSV <- read_csv(file.path(dataDir, "datetimes-numeric.csv"), 
  ...
  mutate(
    ## Using another library here because it's way easier. But any way you shake it, there may
    ## be some seconds being messed with because of conversions
    datetime = force_tz(openxlsx::convertToDateTime(datetime), tzone = "America/Los_Angeles")
  )
...
mixedDatetimesCSV <- read_csv(file.path(dataDir, "datetimes-mixed.csv"), 
  ...
  mutate(
    datetime = parse_date_time(datetime, orders = formats, tz = "America/Los_Angeles")
  )
...
mixedNumericDatetimesCSV <- read_csv(file.path(dataDir, "datetimes-mixed-numeric.csv"), 
  ...
  mutate(
    # This will cause some warnings, so make sure everything parses correctly
    datetime = if_else(
      !is.na(force_tz(openxlsx::convertToDateTime(datetime), tzone = "America/Los_Angeles")),
      force_tz(openxlsx::convertToDateTime(datetime), tzone = "America/Los_Angeles"),
      parse_date_time(datetime, orders = formats, tz = "America/Los_Angeles")
    )
  )
...
simpleDatetimesCSVR <- simpleDatetimesCSVR %>%
  mutate(
    datetime = force_tz(datetime, tzone = "America/Los_Angeles")
  )
